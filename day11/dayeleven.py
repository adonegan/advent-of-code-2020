...
def get_adjacent_count(row,col):
    # need to check space above, below, 
...
def get_adjacent_count_p2(row,col):
    count = 0
    ...
    for row in range(len(data)):
        ...
        for col in range(len(currentRow)):
            ...
            if tolerance == 4:
                ...
                adjacentCount = get_adjacent_count_p2(row,col)
...
def get_final_count(tolerance):
    prev = data.copy() # if we modify data, it doesn't modify prev
    run_rules(tolerance)
...
# Codealong with Dylan Codes: https://www.youtube.com/watch?v=bgfuzs8aBro